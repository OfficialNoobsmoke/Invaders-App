...
export const seedDatabase = async () => {
  ...
  const realmServersResults = await Promise.all([
    db
      .insert(realmServers)
      .values({
  ...
  const realmServersIds = realmServersResults
  ...
  const instanceIds = instanceResults
    .map((result) => result[0]?.id)
    .filter((id): id is string => !!id);
  ...
  // Insert combinations into instancesRealmServers
